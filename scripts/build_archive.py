...
def archive_rail(current):
    ...
    return ''.join(parts)
def shell(current, title, content, active='latest', description='Amir’s daily physics reading archive.', extra=''):
...
home += '<div class="bottom-note">Automatic daily publishing is not connected yet.</div>'
...
def archive_page(current,title,subset,crumb=''):
    ...
    for year,months in groups.items():
        ...
        content+='</section>'
    content+='<p class="bottom-note">Reports for 12–16 September 2026 were imported from the supplied journal-club export.</p>'
